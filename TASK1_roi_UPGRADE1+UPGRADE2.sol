...
contract ROI_Platform{
    ...
    function invest(address L1_refer) public payable
    {
        ...
        investors[msg.sender]=Investor(payable(msg.sender),invested_amount,invested_date,0,L1_refer,true);
        payable(L1_refer).transfer(investors[msg.sender].invested_amount*3/100);
        address L2_refer;
        address L3_refer;
    
        if(investors[L1_refer].is_refered==true)
        {
             L2_refer=investors[L1_refer].refered_address;
            payable(L2_refer).transfer(investors[msg.sender].invested_amount*2/100);
        }
        if(investors[L2_refer].is_refered==true)
        {
             L3_refer=investors[L2_refer].refered_address;
            payable(L3_refer).transfer(investors[msg.sender].invested_amount*1/100);
        }
    }
